...
### "동기화가 끝나지 않음 (진행 로그가 멈춘 채 계속 돌아감)"

```
+ 어떤 노트.md [102/268] created
(이후 아무 출력 없음)
```

**원인 1 — 재시도 대기(가장 흔함)**: Notion이 429(rate limit)나 5xx를 돌려주면 백오프
후 재시도합니다. 대기 중에는 새 출력이 없으므로 멈춘 것처럼 보입니다. 이제는 재시도가
경고로 남으므로 로그로 바로 확인할 수 있습니다:

```
Notion API 재시도 1/5 — 3750ms 대기 (status 429 · rate_limited · ...)
```

이 줄이 계속 보이면 정지가 아니라 **속도 조절 중**입니다. 자주 뜬다면 동시성을 낮추세요
(`advanced.concurrency`, 기본 3). 429는 워크스페이스 전체에 걸리는 신호라 한 건이라도
맞으면 나머지 요청도 같은 시간만큼 함께 쉽니다 — 이게 재시도 폭풍을 막습니다.

**원인 2 — 멈춘 연결**: 특정 페이지 처리 중 응답이 오다 멈춘 연결에 걸린 경우입니다.
이때 프로세스는 CPU도 쓰지 않고 오류도 내지 않아, 겉으로는 "느린 것"과 구분되지 않습니다.

**현재 동작**: 세 겹의 상한이 걸려 있어 무한정 멈추지 않습니다.
...
상한을 넘긴 페이지만 실패로 끊고 나머지는 계속 진행하며, 마지막에 실패 목록으로
`시간 상한 초과(...초): pull <경로>` 형태로 보고합니다. **어느 페이지에서 멎었는지가
그 메시지에 남습니다.**

**해결**: 실패로 보고된 페이지를 Notion에서 확인하세요(대용량 첨부·깨진 파일 블록이
흔한 원인). 느린 회선이라 정상 페이지가 상한에 걸린다면 값을 늘리세요:

```jsonc
{ "advanced": { "itemTimeoutMs": 3600000, "mediaDownloadTimeoutMs": 900000 } }
```
