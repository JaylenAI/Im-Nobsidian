...
> 출발했고, 이번 교훈은 결함 자체보다 **왜 안 보였나**에 있다.
>
> R13 — P11 의 교훈대로 게이트를 넓히자(하니스가 본문을 읽고, DB↔FS 를 양방향으로 대조)
> **그 자리에서 나온** 결함. 없던 결함이 생긴 게 아니라 볼 수 있게 됐다.

### Fixed

- **볼트에서 지운 DB 행이 영영 돌아오지 않던 문제 (R13)** — `pullDatabase` 의 행 루프는 원격 `last_edited_time` 이 마지막 동기화와 같으면 **로컬 파일이 있는지 묻지도 않고** `continue` 했다. 복원 판정 코드(`pullDatabasePage` 안의 `localExists` → `resolvePullConflict` → write)는 멀쩡히 있었지만 **도달하지 못했다**. 한편 오케스트레이터의 페이지 복원 경로(`detectMissingLocalFiles`, R0)는 db-row 를 일부러 제외하면서 이유를 "행은 database-syncer 가 같은 복원 판정을 이미 거치므로"라고 적어 두었다 — **그 전제가 틀렸으므로 양쪽 다 "상대가 한다"고 믿어 아무도 안 했다**(R9a·R9e·R10-D·R11-A 와 같은 결함군: 같은 계약이 두 경로에 있고 한 경로만 그것을 안 지킨다). 노출면은 다수 쪽이다 — 지정 볼트 1,268 레코드 중 **925개(73%)가 db-row**. 게다가 모든 초록 신호와 공존한다: `analyze` 는 FS→DB(고아) 한 방향만 봤고, `verify` 는 **상태 DB 의 id 집합**끼리 맞춰 레코드만 있으면 925=925 이며, `repull`/`sync` 의 churn 은 그 행을 건너뛰니 애초에 0 이다. 3차 전량 E2E 의 `roundtrip`(프로브 폴더 `rm -rf` 후 재 pull)에서 **페이지 28개는 `✓ 28 restored` 로 돌아오고 db-row 1개만 돌아오지 않는** 형태로 드러났다. 봉합은 넷이다 — ① 존재 확인을 스킵 조건에 넣는다(선택적 predicate 주입은 버렸다 — "같은 계약을 두 경로가 나눠 갖고 한쪽이 잊는" 바로 그 실패 모드를 재생산한다. 비용은 pull 당 순차 `stat` 925회로 580초 pull 대비 무시 가능) ② 복원을 `updated` 와 가른다(`DatabaseSyncResult.restored`) — 합쳐 세면 dry-run 과 멱등성 게이트가 "원격이 바뀌었다"고 거짓말하고, 페이지 경로는 이미 같은 이유로 `restored` 를 분리 보고한다 ③ 페이지 변경 0건인 **조용한 pull** 에서 설정된 DB(`notion.databases`)가 하나라도 있으면 `pullAll()` 도 디스커버리도 거치지 않고 `emptyResult` 로 빠져나가던 경로를 막았다 — ①만으로는 그 경로에서 여전히 복원이 안 된다 ④ `analyze.mjs` 에 **DB→FS 축**을 세워 추적 레코드인데 실물이 없으면 위반으로 판정한다. 실피해는 프로브 네임스페이스 1건, **실콘텐츠 0건**(전수 대조 확인)

- **코드 내용 속 백틱이 블록 경계와 구분되지 않아 본문이 코드로 삼켜지던 문제 (P11)** — NFM 은 코드펜스의 **경계**(여는·닫는 펜스)만 탭으로 들여쓰고 **내용**은 열 0 에 둔다(비대칭 들여쓰기 — 들여쓰기 그 자체가 경계 표시다). 그런데 볼트로 쓸 때는 경계를 열 0 으로 평탄화해야 한다(인용 접두 `> ` 가 붙은 뒤에도 Obsidian 의 "4칸 이상은 코드블록" 규칙에 걸리지 않으려면). 평탄화하는 순간 경계 펜스와 **내용 안의 열 0 백틱**을 구분할 근거가 사라지고, Obsidian 이 펜스를 순서대로 잘못 짝지어 그 뒤 본문 전체를 코드블록 안으로 빨아들인다. 실측(사용자 지목 노트 `Empowerment (1)/Blog.md`): 원본 코드블록 **5개 → 볼트 37개**, 본문 **8,200행**이 코드로 삼켜짐. 볼트 전수(1,267 → 수정 후 1,268노트)로는 **미닫힘 펜스 1 → 0 · 코드 안에 갇힌 구조행(제목·콜아웃·표행) 537 → 469 · 원본 NFM 1:1 오라클 2노트 → 0(대조 가능 120노트 전량)**. 초기 보고의 "23노트 파손"은 **취소한다** — 그 23 은 원본 대조 없는 넓은 휴리스틱(`미닫힘 ‖ 최대 코드런 ≥200 ‖ 코드 안 구조행 ≥3`)이고, 조건 `코드런 ≥200` 이 정상적인 긴 코드블록을 그대로 세므로 **수정본 볼트에서도 같은 23 이 나온다**. 파손 개수의 정본은 오라클뿐이다. 봉합은 넷이다 — ① `findFenceClose` 가 **들여쓰기까지** 대조해 내용 펜스가 구조 블록을 닫지 못하게 한다(단 NFM 이 닫는 줄의 들여쓰기를 흘리는 사례가 실제로 있어 — 여는 줄 `\t\t`, 닫는 줄 열 0 — 정확 일치 우선·느슨한 폴백으로 둔다. 폴백이 없으면 멀쩡한 블록이 파일 끝까지 열린 것으로 잡힌다) ② `widenAmbiguousFences` 가 경계 펜스를 **내용의 가장 긴 펜스보다 한 틱 길게** 내보내, 평탄화 뒤에도 CommonMark 의 "여는 펜스 이상만 닫는다" 규칙이 경계를 지켜 준다 ③ 토글 헤딩의 자식 구간 스캔이 컨테이너를 **통째로** 삼킨다(`containerBlockEnds`) — 줄 단위로 들여쓰기를 보면 열 0 인 **닫는 펜스**만 구간 밖으로 밀려나고, 그 자리에 끝 마커가 사용자 코드 **안으로** 끼어든다 ④ `convertNotionTables` 가 코드 내부 오프셋을 건너뛴다 — 코드블록에 적어 둔 `<table>` 예제가 구조로 오인돼 통째로 사라지고 있었다(실측: 한 노트에서 `<table` 29→4 · `<tr` 158→1)
...
- **릴리스 게이트 (v0.3.2 태그 직전 실측)** — `pnpm build → lint → typecheck → format:check → test:coverage` 전부 통과, **단위 테스트 1,662 통과 · 11 skip**(테스트 파일 130 통과 · 3 skip). 실 워크스페이스 대상 **라이브 불변식 10파일 15케이스 통과**(140.3s, exit 0). 산출 볼트 **렌더 린트 1,268노트 스캔 · 결함 0건**. 토글 전용 실측 — 115노트에 토글 콜아웃 **910개**, 토글 헤딩 512개, 토글 안에 중첩된 코드블록 794개·표 행 373개, 원시 `<details>` / `<summary>` / `{toggle="true"}` / NFM 컨테이너 태그 누수 **0**. "빈 토글" 후보 4건은 전건 Notion 원본을 조회해 **원본에서도 비어 있음**을 확인했다(2건은 자식 없는 `<details>`, 1건은 `<empty-block/>` 뿐, 1건은 의도된 프로브 픽스처) — 내용 소실 **0**
- **코드블록 경계 드리프트 지표 (P11)** — 기존 렌더 게이트 7종은 P11 파손을 **구조적으로** 통과시킨다. 삼켜진 본문은 **사라지지 않고** 코드블록 **안에** 그대로 남기 때문이다 — 산문 보존율·글자 수·본문 삼킴·왕복 무손실이 전부 맞다. 실제로 사용자가 지목한 노트는 게이트 코퍼스 **안에 있었고** 게이트는 7/7 초록이었으며, 2차 전량 E2E 도 `analyze CLEAN · repull 멱등 · roundtrip 무손실 · ✓ E2E 전체 통과`를 찍었다(그 시점 볼트의 지목 노트는 코드블록 37개로 부풀어 본문 8,200행을 삼킨 채였다). 그래서 **경계 자체**를 8번째 지표로 못박는다 — 원본과 볼트의 **코드블록 개수·코드 행수**를 대조한다(`codeBoundaryDrift`). 오라클은 구현을 재사용하지 않고 **독립적으로** 짰다. 구현을 빌려 쓰면 구현이 틀린 방식 그대로 틀려서 오라클이 아니게 되며(P11 을 잡아낸 것이 바로 이 독립성이다), 게다가 `classifyContainerLines` 는 **표 내부도 `code` 로 표시**해 코드 전용 지표에는 잘못된 도구다. 오라클의 닫는 펜스 판정도 구현과 같은 이유로 정확 일치 우선·느슨한 폴백이다 — 엄격만 두면 NFM 의 비대칭(여는 `\t\t` · 닫는 열 0)에서 멀쩡한 블록이 EOF 까지 열린 것으로 잡혀 거짓 적색이 난다. **지표 발화 확인**: 수정 前 빌드가 만든 실제 파손 볼트에 물리면 `코드블록수 5→37 · 코드행수 9695→9188` 로 반응하고, 수정본에는 무반응 — 죽은 지표의 0 과 깨끗한 코퍼스의 0 은 출력이 같아 구분이 필요하다. **A/B 실측**은 수정 커밋 직전(`d6fccca^`)과 대조해 실코퍼스 120노트 2 → 0 · 합성 8노트 1 → 0(합계 **3 → 0**)이다. 별도로 **왕복 수렴**도 봤다 — 경계 펜스를 한 틱 넓히는 이상 push 가 그걸 못 되돌리면 왕복마다 백틱이 자란다. 판정은 `roundtrip-convergence.test.ts` 와 같은 계약(1회 정규화 허용 · **2회차부터 고정점**)이며 실코퍼스 전체 파이프라인 왕복에서 1회차 정규화 79노트 · **비수렴 0노트**, 경계 드리프트는 pull·왕복 양방향 **0**
