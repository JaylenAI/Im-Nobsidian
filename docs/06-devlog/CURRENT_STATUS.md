...
> 마지막 업데이트: 2026-07-28
> 버전: v0.3.1 릴리스 완료 · **R0~R12 실데이터 충실도 트랙 개발 완료(dev, 미태그)**
...
- **전체**: **1551개 통과** (11 skip, 0 실패) — core 1356 / cli 46 / plugin 149
- **TypeScript 타입 체크**: 클린 (core·cli·plugin 에러 0) · **eslint 0**
- **플러그인 빌드**: 695KB, sql-wasm.wasm 644KB 별도
- **실데이터 E2E**: clean-slate 볼트 fresh full pull **1267 파일(db-row 925 + 파일 270 + folder-note 72) / 0 실패**
  — audit-vault 결함 0·해시 불일치 0·churn-0 PASS, 재 pull "no changes"(byte-identical), 왕복 수렴, 충돌 0
  · **완결성 PASS(원격 925행 = 볼트 925행, database 166개)** ← R11-B 신설 게이트
