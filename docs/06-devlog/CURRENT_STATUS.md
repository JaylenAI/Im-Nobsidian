...
**v0.1.9 릴리스.** Obsidian 플러그인 프로덕션 레디 (Phase 1-5 완료). sql.js WASM 어댑터, 동기화 사이드바, 양방향 변경 감지, DB 뷰 6종, 진행률 바, 취소 기능.

| 항목                     | 상태                                        |
| ------------------------ | ------------------------------------------- |
| npm `@im-nobsidian/core` | v0.1.9                                      |
| npm `im-nobsidian` (CLI) | v0.1.9                                      |
| GitHub Release           | v0.1.9 tagged                               |
| Obsidian Plugin          | v0.1.9 (BRAT 설치 가능, 커뮤니티 제출 예정) |
...
| Phase 6 | Notion DB → .base 자동 생성 | 📋 예정 | 0%     |
...
| 인라인 편집 (Table)           | ✅      |
| 충돌 해결 모달                | ✅      |
| Notion DB → .base 자동 생성   | 📋 예정 |

## 다음 목표

1. **Phase 6** — Notion DB → Obsidian Bases `.base` 파일 자동 생성 (네이티브 뷰 렌더링)
2. **Phase 7** — 테스트 강화 (플러그인 50+ 테스트)
3. **Phase 8** — 문서 + BRAT 베타 + 커뮤니티 플러그인 제출
