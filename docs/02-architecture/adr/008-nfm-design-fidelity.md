...
## 아이콘 보존 범위 (2026-07-16 P5 실측 보강)

콜아웃 `icon` 속성은 네 가지 형태로 실측됐다 — 보존 정책을 형태별로 나눈다:

| icon 형태                            | 예                                    | 정책                                                            |
| ------------------------------------ | ------------------------------------- | --------------------------------------------------------------- |
| 이모지                               | `⚠️`                                  | 마커 왕복 (type 기본 이모지면 마커 생략)                        |
| custom emoji URI                     | `notion://custom_emoji/<space>/<id>`  | 마커 왕복 (안정 URI — churn 없음)                               |
| Notion 내장 아이콘 경로              | `/icons/drafts_lightgray.svg`         | 마커 왕복 (안정 경로)                                           |
| **업로드 이미지 (http(s) 서명 URL)** | `https://prod-files-secure...X-Amz-…` | **마커 제외, 색만 보존** — 서명이 풀마다 바뀌어 churn·만료 오염 |

업로드 이미지 아이콘은 NFM 이 만료 서명 URL 로만 노출하므로 왕복 보존이 불가능하다.
push 시 해당 콜아웃은 아이콘 없음(색만)으로 degrade 된다.

또한 콜아웃이 컬럼을 **품는** 경우(콜아웃 첫 자식이 columns) innermost 평탄화 순서상
start 마커가 콜아웃 제목으로 흡수되던 것을 실측(인사이드 아웃 외 4파일) — quote 줄 안
인라인 컬럼 마커와 이중 중첩의 구조적 탭(`> > \t%%..%%`)까지 strip 해 평탄화 degrade
로 수렴시킨다.
