 ...
 *
 * 여기에는 **원본 NFM 과 견주는** 지표만 남는다(구조 드리프트·본문 보존·코드블록 경계).
 * 회귀 코퍼스를 쥔 테스트만 할 수 있는 판정이다. 파일 하나만 보고 판정하는 ①~⑧ 은
 * 제품 쪽 {@link lintRendered} 에 있고 동기화 하니스도 같은 것을 쓴다 — 게이트와 하니스가
 * 다른 잣대를 들면 한쪽이 초록인 채 다른 쪽이 깨진다.
 *
 * 규칙 번호(①~⑰)는 트랙 초기 실측 조사에서 붙인 결함 분류를 그대로 잇는다.
 */

import { SEPARATOR_BODY } from "../../src/audit/render.js";
...
export type {
  RenderFinding as Finding,
  RenderLineRule as LineRule,
} from "../../src/audit/render.js";
