...
| `nobsi verify`      | DB 완결성 검증 — 원격 행이 볼트에 빠짐없이 있는가 (읽기 전용) |
...
database 별로 원격 행 id **집합**과 볼트가 추적 중인 db-row id **집합**을 직접 대조하고, Notion에는
있는데 로컬에 없는 행이 하나라도 있으면 0이 아닌 코드로 끝납니다. 멱등성 검사(`pull`을 다시 돌려
변경 0을 확인하는 것)로는 이걸 잡을 수 없습니다 — 디스커버리가 매번 **같은 행을 똑같이** 놓치면
재실행 결과도 첫 실행과 같기 때문입니다. CI 용으로는 `--json`을 붙이세요.
